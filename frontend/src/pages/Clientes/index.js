...
const Clientes = ({ usuario }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`https://locatech-backend.onrender.com/clientes/${editingId}`, dataToSend);
        alert('Cliente atualizado com sucesso!');
      } else {
        await axios.post('https://locatech-backend.onrender.com/clientes', dataToSend);
  ...
  const handleEdit = async (id) => {
    try {
      const response = await axios.get(`https://locatech-backend.onrender.com/clientes/${id}`);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Tem certeza que deseja excluir este cliente?')) {
      try {
        await axios.delete(`https://locatech-backend.onrender.com/clientes/${id}`);
